...
This function does simple data cleaning. Importantly, records are discarded if first name, last name, birth date, address, state of residence, OR city AND zip are missing.
...
Sometimes we receive HIP records that should be processed in a future season. We also sometiems receive records that should be processed in both this season and a future one, since some states have license years that encompass two seasons. The `issueCheck` function evaluates issue_date and registration_yr for two-season states (ME, MA, ID, CT, WV, VT, NH, IA, ND, and TN) as well as a 365-day state (MS). It also sets aside records from any state that have future registration_yr value.

Note: Records "set aside" will be filtered out of the current data and written to a future data .csv file to be accessed in the next HIP season.

For ME, MA, ID, CT, WV, VT, NH, IA, ND, and TN:
...
```r
DL1301_current <- issueCheck(DL1301_clean, year = 2021, write = F)
```
...
```
## 6615 records have been postponed for next year. They will be filtered out from this download and saved in the future data .csv with their original registration_yr.
```

To export a table of records to process in a future season, specify the outpath where the .csv should be written and specify `write = TRUE`.


```r
DL1301_current <- issueCheck(DL1301_clean, year = 2021, path = "C:/HIP/future_data/", write = T)
```
...
##  1 address                                          1
##  2 address-email                                    1
##  3 address-zip-email                                1
##  4 address-zip-issue_date                           1
##  5 bag                                            483
##  6 email                                            1
##  7 hunt_mig_birds                                   1
##  8 issue_date                                      13
...
## 11 issue_date-hunt_mig_birds-dl_date                3
...
3. For duplicates from sea duck states (AK, CA, CT, DE, MA, MD, ME, NC, NH, NJ, NY, RI, VA), keep records with a 2 in seaduck.
4.  For duplicates from brant states (AK, CA, CT, DE, MA, MD, NH, NJ, NY, NC, RI, VA), keep records with a 2 in brant.
...
##    <chr>    <chr>              <chr>        <chr>        
##  1 ND       woodcock_bag       0            1, 2, 3, 5   
##  2 OR       dove_bag           0            1, 2, 3, 5   
##  3 ME       dove_bag           0            1, 2, 3, 4   
##  4 OR       ducks_bag          0            1, 2, 3, 4   
##  5 OR       geese_bag          0            1, 2, 3, 4   
##  6 OK       cranes             0            1, 2         
##  7 OR       band_tailed_pigeon 0            1, 2         
##  8 OR       brant              0            1, 2         
##  9 OR       seaducks           0            1, 2         
## 10 CO       seaducks           0            1            
## # … with 204 more rows
...
##  2 WI20211229.txt 1         273     323       0.845
##  3 MO20211229.txt 1        2697    3604       0.748
##  4 ME20211229.txt 1         112     151       0.742
##  5 NM20211229.txt 1         208     295       0.705
##  6 ID20211229.txt 1        1761    2588       0.680
##  7 ND20211215.txt 1         130     204       0.637
##  8 CA20211229.txt 1        2003    3211       0.624
##  9 IN20211229.txt 1         416     690       0.603
## 10 GA20211229.txt 1        2199    3821       0.576
## # … with 33 more rows
```
...
##    <chr>        <chr>      <chr> <chr>   
##  1 record_8188  20589      CO    DC      
##  2 record_9556  32578-2725 OH    FL      
##  3 record_10308 32428-6908 VA    FL      
##  4 record_12168 85034      GA    AZ      
##  5 record_14484 32435-6458 GA    FL      
##  6 record_16247 2034       GA    DC      
##  7 record_16261 2034       GA    DC      
##  8 record_18352 35213-3909 GA    AL      
##  9 record_18432 30161-4073 WY    GA      
## 10 record_19442 98409      IA    WA      
## # … with 102 more rows
```
...
## ! Aesthetics must be either length 1 or the same as the data (1): x, y and label
```
...
##   <chr>           <int>         <int>      <dbl> <chr>       
## 1 CT                257          5943     0.0415 error > 0.03
```
...
##   <chr>             <int>         <int>      <dbl> <chr>       
## 1 birth_date         2157         39705     0.0515 error > 0.01
```
