...
* Creates a record_key column (unique identifier for each row, used later by `shiftFix`)

We will use the default settings to read in all of the states from download 1202.
...
snoop <-
  ...
  mutate(n_shift = str_length(str_extract(issue_date, "^[0-9]+(?=\\/)")) - 2) %>%
...
```
## Error: Problem with `mutate()` column `n_shift`.
## i `n_shift = str_length(str_extract(issue_date, "^[0-9]+(?=\\/)")) - 2`.
## x could not find function "str_length"
```
...
```
## Error in map(.x, .f, ...): object 'snoop' not found
```
...
## i Input `..1` is `!record_key %in% unshifted$record_key`.
## x object 'unshifted' not found
```
...
```r
DL1202_clean <- clean(DL1202_unshifted)
```

```
## Error in filter(., !is.na(firstname)): object 'DL1202_unshifted' not found
```
...
* Address cleaning
    ...
    * Attempt at P.O. Box standardization, by changing "P.O.BOX" to "P.O. BOX" and "PO BOX" to "P.O. BOX"
...
## 10 issue_date-hunt_mig_birds     3
## 11 middle-address                2
## 12 middle-address-email          2
## 13 middle-issue_date             1
## 14 middle-issue_date-email       1
## 15 middle-suffix                 1
## 16 suffix-issue_date             1
## 17 zip                           1
```
...
##    <chr>       <chr> <chr> <chr>   
##  1 record_186  32525 AL    FL      
##  2 record_224  34044 AL    AA      
##  3 record_1829 15425 AL    PA      
##  4 record_3297 63804 AL    MO      
##  5 record_5219 36564 MS    AL      
##  6 record_6311 79556 FL    TX      
##  7 record_6984 65466 AL    MO      
##  8 record_4    44023 AR    OH      
##  9 record_44   38655 TN    MS      
## 10 record_57   76182 AR    TX      
## # ... with 507 more rows
```
...
## 2 email              6516        114670     0.0538 error > 0.01
