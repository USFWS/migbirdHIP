...
```
## Error in DL1301 %>% filter(!str_detect(birth_date, "^[0-9]{2}\\/[0-9]{2}\\/[0-9]{4}$")) %>% : could not find function "%>%"
```
...
```
## Error in map_dfr(1:length(snoop$record_key), ~shiftFix(DL1301, snoop$record_key[.x], : could not find function "%>%"
```
...
```
## Error in DL1301 %>% filter(!record_key %in% unshifted$record_key) %>% : could not find function "%>%"
```
...
```
## Error in filter(., !is.na(firstname)): object 'DL1301_unshifted' not found
```
...
```r
findDuplicates(DL1301_clean)
```

```
## Error in tbl_vars_dispatch(x): object 'DL1301_clean' not found
```


```r
findDuplicates(DL1301_clean, return = "table")
```
...
```
## Error in group_by(., firstname, lastname, state, birth_date, dl_state, : object 'DL1301_clean' not found
```
...
```
## Error in select(., source_file, ducks_bag, geese_bag, dove_bag, woodcock_bag): object 'DL1301_fixed' not found
```
...
```
## Error in DL1301_fixed %>% mutate(ducks_bag = ifelse(dl_state == "AK", : could not find function "%>%"
```
...
```r
DL1301_corrected <- correct(DL1301_proofed, year = 2021, data = "bag")
```
