...
Sometimes we receive HIP records that should be processed in a future season. We also sometiems receive records that should be processed in both this season and a future one, since some states have license years that encompass two seasons. The `issueCheck` function evaluates issue_date and registration_yr for two-season states (ME, MA, ID, CT, WV, VT, NH, IA, ND, and TN) as well as a 365-day state (MS).
...
```
## Error in `mutate()`:
## ! Problem while computing `issue_id = case_when(...)`.
## Caused by error in `mdy(issue_date) %within% interval(issue_start, last_day_migbird_hunting)`:
## ! could not find function "%within%"
```
...
```
## Error in tbl_vars_dispatch(x): object 'DL1301_current' not found
```
...
```
## Error in group_by(., firstname, lastname, state, birth_date, dl_state, : object 'DL1301_current' not found
```
...
```
## Error in select(., dl_state, ducks_bag:seaducks): object 'DL1301_fixed' not found
```
...
```
## Error in select(., source_file, ducks_bag, geese_bag, coots_snipe): object 'DL1301_fixed' not found
```
...
```
## Error in mutate(., ducks_bag = ifelse(dl_state == "AK", 3, ducks_bag)): object 'DL1301_fixed' not found
```
...
```
## Error in select(., dl_state, dl_date, ducks_bag): object 'DL1301_fixed' not found
```
...
```
## Error in select(., dl_state, {: object 'DL1301_fixed' not found
```
...
```
## Error in mutate(., temp_key = paste0("row_", row_number())): object 'DL1301_fixed' not found
```
...
```
## Error in mutate(., errors = ifelse(is.na(errors), "none", errors), title = ifelse(str_detect(errors, : object 'DL1301_proofed' not found
```
...
```
## Error in select(., error, errors): object 'DL1301_proofed' not found
```
...
```
## Error in select(., state, dl_state): object 'DL1301_proofed' not found
```
...
```
## Error in group_by(., dl_state): object 'DL1301_proofed' not found
```
...
```
## Error in select(., errors): object 'DL1301_proofed' not found
```
...
```
## Error in filter(., dl_state == loc): object 'DL1301_proofed' not found
```
...
```
## Error in mutate(., birth_year = str_extract(birth_date, "(?<=\\/)[0-9]{4}$"), : object 'DL1301_proofed' not found
```
...
```
## Error in select(., errors, dl_state): object 'DL1301_proofed' not found
```
