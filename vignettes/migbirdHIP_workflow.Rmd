...
snoop <-
  DL1301 |>
  dplyr::filter(
    !stringr::str_detect(birth_date, "^[0-9]{2}\\/[0-9]{2}\\/[0-9]{4}$")) |>
  dplyr::mutate(
    n_shift =
      stringr::str_length(
        stringr::str_extract(issue_date, "^[0-9]+(?=\\/)")) - 2) |>
  dplyr::relocate(record_key, .before = "title") |>
  dplyr::relocate(source_file, .before = "record_key") |>
  dplyr::select(record_key, n_shift) |>
  print()
```
...
```
## # A tibble: 16 × 3
##    source_file    bad_zip  prop
##    <chr>            <int> <dbl>
##  1 MO20211229.txt      39 0.011
##  2 NE20211217.txt      15 0.017
##  3 AL20211229.txt       7 0.001
##  4 NE20211222.txt       6 0.014
##  5 LA20211217.txt       5 0.008
##  6 ID20211229.txt       4 0.002
##  7 WA20211229.txt       4 0.003
##  8 IA20211215.txt       3 0.001
##  9 OK20211229.txt       3 0.001
## 10 WA20211215.txt       3 0.002
## 11 UT20211229.txt       2 0.003
## 12 FL20211229.txt       1 0    
## 13 ME20211229.txt       1 0.007
## 14 NM20211229.txt       1 0.003
## 15 OR20211229.txt       1 0.001
## 16 TN20211229.txt       1 0
```
...
```
## * 0 future records need to be modified (registration_yr - 1) and copied for next year.
```
...
```
## * 709 future records have been postponed for next year. They will be filtered out from this download and saved in the future data .csv with their original registration_yr.
```
...
##   <chr>       <chr>            <chr>    <int>
## 1 nochange    past             bad      30023
## 2 nochange    two season state nochange 11655
## 3 postpone    past             bad       6616
## 4 postpone    two season state postpone   709
```

```
## * Future data write-out skipped.
```
...
##  3 OR       dove_bag     0            1, 2, 3, 5       11 1%   
##  4 TN       dove_bag     0            1, 2, 3, 5       85 4%   
##  5 TN       woodcock_bag 0            1, 2, 3, 5       85 4%   
##  6 IA       ducks_bag    0            1, 2, 3, 4     2841 89%  
##  7 IA       geese_bag    0            1, 2, 3, 4     2841 89%  
##  8 OR       ducks_bag    0            1, 2, 3, 4       11 1%   
##  9 OR       geese_bag    0            1, 2, 3, 4       11 1%   
## 10 TN       ducks_bag    0            1, 2, 3, 4       85 4%   
## # ℹ 31 more rows
```
...
```
## # A tibble: 23 × 5
##    source_file    spp1               spp2                 n value  
##    <chr>          <chr>              <chr>            <int> <chr>  
##  1 CT20211229.txt coots_snipe        dove_bag            87 1      
##  2 IA20211215.txt brant              woodcock_bag      3175 0, 1, 2
##  3 IA20211215.txt brant              rails_gallinules  3175 0, 1, 2
##  4 IA20211215.txt rails_gallinules   woodcock_bag      3175 0, 1, 2
##  5 KY20211228.txt band_tailed_pigeon cranes             984 1      
##  6 KY20211228.txt band_tailed_pigeon brant              984 1      
##  7 KY20211228.txt band_tailed_pigeon seaducks           984 1      
##  8 KY20211228.txt brant              cranes             984 1      
##  9 KY20211228.txt brant              seaducks           984 1      
## 10 KY20211228.txt cranes             seaducks           984 1      
## # ℹ 13 more rows
```
...
##  7 ID20211229.txt 1        1760    2588       0.680
##  8 CT20211229.txt 1          57      87       0.655
##  9 ND20211215.txt 1         130     204       0.637
## 10 CA20211229.txt 1        2003    3211       0.624
## # ℹ 37 more rows
```
...
##   <chr>           <int>         <dbl>      <dbl> <chr>       
## 1 CT                 88          1130     0.0722 error > 0.03
## 2 AL               4814         62260     0.0718 error > 0.03
## 3 LA               1808         23392     0.0717 error > 0.03
```
...
##   <chr>                  <int>         <int>      <dbl> <chr>       
## 1 registration_yr         6592         41161     0.138  error > 0.01
## 2 birth_date              1295         46458     0.0271 error > 0.01
```
