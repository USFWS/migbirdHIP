...
Sometimes we receive HIP records that should be processed in a future season. We also sometiems receive records that should be processed in both this season and a future one, since some states have license years that encompass two seasons. The `issueCheck` function evaluates issue_date and registration_yr for two-season states (ME, MA, ID, CT, WV, VT, NH, IA, ND, SD, and TN) as well as a 365-day state (MS). It also sets aside records from any state that have future registration_yr value.
...
```
## 695 records have been modified to have registration_yr - 1. They have been copied and saved for next season in the future data .csv with their original registration_yr.
```

```
## 5672 records have been postponed for next year. They will be filtered out from this download and saved in the future data .csv with their original registration_yr.
```
...
##  1 address                               1
##  2 address-email                         1
##  3 address-zip-issue_date                1
##  4 bag                                 432
##  5 email                                 1
##  6 hunt_mig_birds                        1
##  7 issue_date                           10
##  8 issue_date-dl_date                    1
##  9 issue_date-email                      1
## 10 issue_date-hunt_mig_birds-dl_date     2
```
...
##    <chr>          <chr>   <int>   <int>       <dbl>
##  1 WI20211229.txt 1         230     275       0.836
##  2 IA20211215.txt 0        1376    1647       0.835
##  3 ME20211229.txt 1          91     119       0.765
##  4 MO20211229.txt 1        2275    3150       0.722
##  5 NM20211229.txt 1         164     240       0.683
##  6 ID20211229.txt 1        1390    2110       0.659
##  7 CA20211229.txt 1        1341    2191       0.612
##  8 ND20211215.txt 1         115     188       0.612
##  9 IN20211229.txt 1         371     630       0.589
## 10 KY20211228.txt 1         486     844       0.576
## # … with 32 more rows
```
...
##   <chr>             <int>         <int>      <dbl> <chr>       
## 1 birth_date         1687         33418     0.0481 error > 0.01
```
