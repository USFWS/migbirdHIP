...
Use `unique = TRUE` to read in a frame without exact duplicates, or `unique = FALSE` to read in all registrations including exact duplicates. **Important:** the `record_key` field is not created if `unique = FALSE`, which is required in some following steps (e.g., `duplicateFix()`, `proof()`).
...
-   `hunt_mig_birds`
    -   For Oregon and/or Washington, if `hunt_mig_birds` is `0` and if `band_tailed_pigeon`, `brant`, or `seaduck` is `2`, change `hunt_mig_birds` field from `0` to `2`
-   `band_tailed_pigeon`
...
The `issueCheck()` function assesses the validity of the `issue_date` field based on the current hunting season's HIP issuance start and end dates (not season open and close dates). A plot is automatically returned for past and future registrations. The plot is skipped by default, so to plot the data write `plot = TRUE`.
...
If a value for a species group is given in the HIP data that doesn't match anything in our records, the species reported in the output will have `NA` values in the `expected_bag_value` column. These species are not hunted in the reported states.
