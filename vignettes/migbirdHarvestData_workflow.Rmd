...
- [Stage 1: Import Data](#stage-1-import-data)
    - [read_hip](#read_hip)
- [Stage 2: Clean Data](#stage-2-clean-data)
    - [tidy](#tidy)
...
- [Stage 3: Explore Errors](#stage-3-explore-errors)
    ...
    - [redFlags](#redflags)
- [Stage 4: Visualize Data](#stage-4-visualize-data)
...
- [Stage 5: Correct Data](#stage-5-correct-data)
    - [correct](#correct)
    - [manualFix](#manualfix)
- [Stage 6: Write and Sample](#stage-6-write-and-sample)
...
## [31mx[39m Columns `12/09/2020`, `12/06/2020`, `12/03/2020`, `12/12/2020`, `12/05/2020`, etc. don't exist.
```
...
States that require special permits to hunt migratory bird species (i.e. cranes, Band-tailed Pigeons, sea ducks and Brant) sometimes send their permit information mixed in with HIP download data. These cases are a common cause of duplicate registrations and are problematic for future analyses if retained. The `fixDuplicates` function attempts to identify such cases and will consolidate hunters with multiple records (i.e. a HIP record and a permit record in the same download state) into one row. The remainder of duplicates that cannot be resolved are moved to a separate tibble.


```r
DL1202_fixed <- fixDuplicates(DL1202_tidied)
```
...
##  9 CO       band_tailed_pigeon 999999       0, 1, 2, 3, 4, 5
## 10 MO       woodcock_bag       9            0, 1, 2, 3, 4, 5
## # ... with 279 more rows
```
...
##   <chr>          <chr>   <int>   <int>       <dbl>
## 1 wi20201216.txt 1         441     546       0.808
## 2 CT20201216.txt 1          68     110       0.618
## 3 NY20201216.txt 1         538     955       0.563
## 4 NE20201209.txt 1         289     547       0.528
## 5 LA20201216.txt 1        1530    3252       0.470
## 6 AR20201216.txt 1        3089    6567       0.470
```
...
##  5 AL       20201216 cranes                   7025
##  6 AL       20201216 band_tailed_pigeon       7025
##  7 AL       20201216 brant                    7025
##  8 AL       20201216 seaducks                 7025
##  9 AR       20201216 cranes                   6567
## 10 AR       20201216 band_tailed_pigeon       6567
## # ... with 99 more rows
```
...
##  5 AL       1202     cranes                   7025
##  6 AL       1202     band_tailed_pigeon       7025
##  7 AL       1202     brant                    7025
##  8 AL       1202     seaducks                 7025
##  9 AR       1202     cranes                   6567
## 10 AR       1202     band_tailed_pigeon       6567
## # ... with 99 more rows
```
...
## 10 firstname                  111
## 11 geese_bag                   38
## 12 hunt_mig_birds              20
## 13 lastname                    10
## 14 rails_gallinules            48
## 15 registration_yr              4
## 16 seaducks                     3
## 17 state                        6
## 18 suffix                     283
## 19 woodcock_bag                 4
...
##   <chr>             <int>         <int>      <dbl> <chr>       
## 1 birth_date         5634         87395     0.0606 error > 0.01
## 2 email              5489         87540     0.0590 error > 0.01
## 3 ducks_bag          3113         89916     0.0335 error > 0.01
## 4 zip                2132         90897     0.0229 error > 0.01
```
...
```r
errorPlot_fields(DL1202_proofed, loc = "LA")
```
...
The year in which the HIP data were collected can also be specified in `errorPlot_fields` to visualize the proportion of "errors" created by youth hunters, since these values fall outside the expected parameters of the `proof` function. It is encouraged to use `specify` when error counts are high to increase transparency by revealing which errors are attributable to a specific cause. This feature should also help maintain confidence in the integrity of the data.
