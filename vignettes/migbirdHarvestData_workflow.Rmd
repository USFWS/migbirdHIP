...
<b>Vertically.</b> The vertical check searches within each column for repetition. Any species group column with a uniform value in all rows will be detected. States that do not have a hunting season for one or more species groups (e.g. seaducks) will not be returned using this function for reporting all zero values.
...
##    <chr>    <chr>    <chr>                  <int>
##  1 AK       _1202    dove_bag                 106
##  2 AK       _1202    woodcock_bag             106
##  3 AK       _1202    rails_gallinules         106
##  4 AK       _1202    band_tailed_pigeon       106
##  5 AL       _1202    cranes                  7073
##  6 AL       _1202    band_tailed_pigeon      7073
##  7 AL       _1202    brant                   7073
##  8 AL       _1202    seaducks                7073
##  9 AR       _1202    cranes                  6569
## 10 AR       _1202    band_tailed_pigeon      6569
## # ... with 102 more rows
```
...
The last intermediate step we do to make sure all of the data are in order is run `strataCheck`. This function searches to find values in species group columns that are not typical or expected by the FWS. If a value outside of the normal strata is given, the output tibble will create a line with the state and species, unusual stratum, and column called "normal_strata" that are the values we would expect. If a value for a species group is given in the HIP data that doesn't match anything in our records, the species reported in the output will have NA values in the "normal_strata" column. These species are not hunted in the reported states.
