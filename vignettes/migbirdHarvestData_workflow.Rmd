...
- [Stage 4: Correct and Compare](#stage-4-correct-and-compare)
    - [correct](#correct)
- [Stage 5: Write Reports](#stage-5-write-reports)
...
##   <chr>                         <chr>           <dbl>
## 1 C:/HIP/DL_1202/MO20201216.txt ISO-8859-1      0.27 
## 2 C:/HIP/DL_1202/NE20201209.txt ISO-8859-1      0.290
## 3 C:/HIP/DL_1202/NE20201209.txt ISO-8859-2      0.22 
## 4 C:/HIP/DL_1202/SC20201202.txt ISO-8859-1      0.28
```

```
## Attention: Directory contains file(s) with non-UTF-8 encoding.
```
...
```
## Attention: File(s) with non-UTF-8 encoding will be re-written.
```
...
```
## Attention: Uniform value detected within one or more fields, please review.
```

```
## # A tibble: 29 x 4
##    dl_state dl_date  v_uniformity                                       n_uniform
##    <chr>    <chr>    <chr>                                                  <int>
##  1 TX       20201216 cranes-band_tailed_pigeon-brant-seaducks               15803
##  2 MS       20201215 cranes-band_tailed_pigeon-brant-seaducks                8929
##  3 SC       20201202 band_tailed_pigeon-brant-seaducks                       7838
##  4 AL       20201216 cranes-band_tailed_pigeon-brant-seaducks                7073
##  5 AR       20201216 cranes-band_tailed_pigeon-brant-seaducks                6569
##  6 MD       20201215 cranes-band_tailed_pigeon                               4650
##  7 OK       20201216 cranes-band_tailed_pigeon-brant-seaducks                4450
##  8 CA       20201216 woodcock_bag-cranes                                     3998
##  9 FL       20201216 geese_bag-cranes-band_tailed_pigeon-brant-seaducks      3657
## 10 GA       20201216 cranes-band_tailed_pigeon-brant-seaducks                3596
## # ... with 19 more rows
```

An important intermediate step is to `validate` the data. This function should be run after `tidy` to assess the data in two dimensions, using `type = "vertical"` to check for repetition vertically within a column (e.g. all values in the geese_bag column are 4) or `type = "horizontal"` to identify repetition horizontally across records (e.g. for one or more hunters, values across their bag and species columns are 9). If a uniformity is detected vertically or horizontally, a tibble containing the offending columns or rows are returned with a warning. If there are no uniformities, a positive message is returned.

Issues returned from the `validate` function can be looked at more closely using `investigate` (see Stage 3: [investigate](#investigate)).
...
<img src="figure/errorfieldsplotsc-1.png" title="Figure 2. Plot of all South Carolina's errors by field name." alt="Figure 2. Plot of all South Carolina's errors by field name." width="500" style="display: block; margin: auto;" />
...
Lastly, for season total data, the plot can be faceted using either dl_cycle or dl_date. In addition, it is possible to add any other ggplot2 arguments to the plot.


```r
errorPlot_fields(DL1202_proofed, specify = 2020) + facet_wrap(~dl_cycle)
```
...
<img src="figure/errorstatesprop-1.png" title="Figure 4. Plot of proportion of error by state." alt="Figure 4. Plot of proportion of error by state." width="500" style="display: block; margin: auto;" />
...
<img src="figure/errorstatescount-1.png" title="Figure 5. Plot of counts of error by state." alt="Figure 5. Plot of counts of error by state." width="500" style="display: block; margin: auto;" />

### findDuplicates

The `findDuplicates` function finds hunters that have more than one record in the download data. Hunters are grouped by first name, last name, city, state, and birth date. If the same hunter has 2 or more records, the fields that are not identical are counted and summarized in a tibble and plot.
...
```
## There are 607 hunters with duplicates; 1230 total duplicated records.
```
...
##    <chr>                               <int>
##  1 address                                 4
##  2 address-dl_state                        5
##  3 address-email                           1
##  4 address-email-dl_state                  4
##  5 address-email-dl_state-dl_date          2
##  6 address-issue_date                      1
##  7 address-issue_date-dl_state            12
##  8 address-issue_date-dl_state-dl_date     2
##  9 address-issue_date-email                1
## 10 address-issue_date-email-dl_state       9
## # ... with 146 more rows
```

### outOfStateHunters

The `outOfStateHunters` function allows us to visualize how many hunters report harvests from a location that does not match the state, province, or territory of their address. This function can plot errors as either proportions or counts by state.


```r
outOfStateHunters(DL1202_proofed, type = "count")
```

<img src="figure/outofstatect-1.png" title="Figure 7. Plot of counts of out-of-state hunters." alt="Figure 7. Plot of counts of out-of-state hunters." width="500" style="display: block; margin: auto;" />

```
## # A tibble: 30 x 2
##    dl_state out_of_state
##    <chr>           <int>
##  1 AK                 43
##  2 AL               1031
##  3 AR               4245
...
```r
outOfStateHunters(DL1202_proofed, type = "proportion")
```

<img src="figure/outofstatepr-1.png" title="Figure 8. Plot of proportions of out-of-state hunters." alt="Figure 8. Plot of proportions of out-of-state hunters." width="500" style="display: block; margin: auto;" />
