...
After the download data are proofed, the next step is to correct the data to the best of our ability. Data can be corrected by running the `correct` function on the proofed tibble.
...
All functions in Stage 2 and Stage 3 will run on the corrected tibble, DL1202_corrected. Errors can be compared between the proofed stage and corrected stage to get a better idea of which errors were serious (i.e., difficult to correct automatically) and determine how the serious errors can be prevented in the future.
