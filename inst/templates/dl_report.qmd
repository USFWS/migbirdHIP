  ...
  DT::datatable(
    ...
    options =
      list(
        ...
        buttons = c("csv", "excel", "pdf"),
        columnDefs = 
          list(list(className = 'dt-right', targets = 5)))
    )
} else {
...
if (bad_issue_nrow > 0) {
  ...
  DT::datatable(
    ...
    options = 
      list(
        columnDefs = 
          list(list(className = 'dt-right', targets = 4)))
  )
}
...
if (nrow(dplyr::filter(issue_assignments, decision == "future")) > 0) {
  DT::datatable(
    ...
    options = 
      list(
        columnDefs = 
          list(list(className = 'dt-right', targets = 3)))
    )
} else {
...
DT::datatable(
  ...
  options = 
    list(
      columnDefs = 
        list(list(className = 'dt-right', targets = 2)))
  )
```
...
DT::datatable(
  migbirdHIP::errorTable(proofed_data, loc = "all") |>
    dplyr::arrange(dplyr::desc(error_count))  |> 
    dplyr::mutate(error_count = format.default(error_count, big.mark = ",", justify = "right")) |> 
    dplyr::rename(
      State = dl_state,
      Error = error,
      Count = error_count), 
