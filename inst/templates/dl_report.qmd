...
important_statistics <- 
  dplyr::bind_rows(
    ...
    dplyr::filter(
      !stringr::str_detect(
        value, 
        "reg_yr_eval|Current registrations with|are blank|ID fields|provided state|write-out|past records|0 future|issue_date in the|Mississippi|100% of emails|00/00/0000|2s converted|0 records need to be postponed|single digit|non-MS records")
    ) |> 
    dplyr::mutate(
      value = 
...
if(is.null(stratacheck_tbl)) {
  ...
  stratacheck_tbl_non0 <- 
    ...
    dplyr::filter(
      !((state_strata == 0 & normal_strata == 1) | 
          (state_strata == 1 & normal_strata == 0))
    )
}
...
if(!is.null(identicalbags)) {
  ...
  idb_filtered <- identicalbags
}

if(!is.null(idb_filtered)) {
...
# stratacheck_tbl_non0
# NULL:       no strata abnormalities
# nrow == 0:  strata abnormalities not a concern
# nrow > 0:   strata abnormalities reported

if (is.null(stratacheck_tbl_non0)) { 
  cat(paste0("✔️", " All bag values are correct."))
} else if(nrow(stratacheck_tbl_non0) == 0) {
  ...
  kableExtra::kbl(
    stratacheck_tbl_non0 |> 
      ...
      dplyr::rename(
        `State` = dl_state,
        Species = spp,
        `Bag provided` = state_strata,
        `Bag expected` = normal_strata,
        Count = n,
        Proportion = prop)) |> 
  ...
  DT::datatable(
    suppressMessages(
      issue_assignments |> 
        ...
        dplyr::ungroup() |>
        dplyr::arrange(dplyr::desc(n)) |> 
        dplyr::rename(State = dl_state, `Number of records` = n)
      )
    )
} else {
