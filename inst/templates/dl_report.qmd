...
# Included to catch messages from qualityMessages, which contains the checks previously run by readMessages() (deprecated) inside of read_hip()
invisible(catch_messages(migbirdHIP::qualityMessages(dl_data, params$year)))
```
...
current_data <-
  catch_messages(migbirdHIP::issueCheck(cleaned_data, params$year))
```
...
# Read in the completed download data (for better/more accurate figures and
# tables)
final_data <-
  purrr::map_df(
    seq_along(list.files(params$temp_path)),
    ~data.table::fread(
  ...
  dplyr::mutate(
    type =
      dplyr::case_when(
        ...
        decision == "past" ~
          paste0(
            "issue date before the first day of issuance (past records",
            " filtered out)"),
        # Issue date is after issue_end (and is not 2 season state)
        decision == "future" ~
          paste0(
            "issue date after end of issue window (future records to be ",
            "sampled next season)")
      )
  ) |>
...
```{r overview_statistics_droppedrecords}
#| echo: false
#| message: false
...
n_dropped <-
  migbirdHIP:::nDropped(
    ...
    deduplicated_data = fixed_data,
    year = params$year,
    by_state = FALSE
  ) |>
  dplyr::mutate(
    n =
      ifelse(.data$n > 999, format.default(.data$n, big.mark = ","), .data$n),
    decision =
      paste("    +", .data$n, .data$decision)) |>
  dplyr::select(-"n")

fixed_data <- NULL
```

```{r overview_statistics_canada}
#| echo: false
#| message: false
...
canada_records <-
  final_data |>
  dplyr::select(state) |>
  dplyr::filter(state %in% migbirdHIP:::REF_ABBR_CANADA) |>
  nrow()
```

```{r overview_statistics_missingemails}
#| echo: false
#| message: false
# Missing emails
emails_missing <-
...
```{r overview_statistics_missingpii}
#| echo: false
#| message: false
...
```{r overview_statistics_issuedate}
#| echo: false
#| message: false
  ...
  dplyr::filter(
    !stringr::str_detect(
      ...
      paste0(
        ...
        as.numeric(migbirdHIP:::REF_CURRENT_SEASON) - 1, "|",
        migbirdHIP:::REF_CURRENT_SEASON, "|",
        as.numeric(migbirdHIP:::REF_CURRENT_SEASON) + 1, ")"))
    ) |>
...
```{r overview_statistics_capturedmessages}
#| echo: false
#| message: false
...
important_statistics <-
  dplyr::bind_rows(
    ...
    unlist() |>
    dplyr::as_tibble() |>
    # Exclude some messages from the bulleted list
    dplyr::filter(
      !stringr::str_detect(
        value,
        paste0(
          "Bad .+ values detected\\.|", # qualityMessages
          "Time to read in|",
          "combinations of PII|", # missingPIIMessage
          "100% of emails|", # missingEmailsMessage
          "Are you sure|", # message for using non-current year
          "00/00/0000|", # zeroDateMessage
          "0 records need to be postponed|", # futureDateMessage
          "0 invalid|", # invalidDateMessage
          "past records|", # pastDateMessage
          "evaluated using registration_yr|", # twoSeasonMessage
          "2-season overlap window|", # twoSeasonMessage
          "issue_date in the future|", # timeTravelMessage
          "2s converted|", # btpiPermitBagFix and cranePermitBagFix
          "are blank|", # dropBlankFiles
          "provided state|" # zipCheck
          )
      )
    ) |>
    dplyr::mutate(
      value = value |>
        stringr::str_remove("^(Warning: |Error: |\\* )") |>
        stringr::str_remove("\\\n$") |>
...
```{r overview_statistics_blankfiles}
#| echo: false
#| message: false
...
```{r overview_statistics_bagcheck}
#| echo: false
#| message: false
...
pmts <-
  ...
  migbirdHIP:::duplicateRecordType() |>
...
bullets <-
  c(
    ...
    paste0(
      ...
      suppressWarnings(
        format.default(
          sum(sumLines(params$raw_path)$num_lines),
          big.mark = ",")
        ),
      ...
      " states (count includes blank lines).")
    ,
    ...
    paste0(
      ...
      " states.")
    ,
    # Report the number of sample eligible registrations
    paste0(
      "* A total of ",
      format.default(
        nrow(proofed_data |> 
               dplyr::filter(registration_yr == params$year)), 
             big.mark = ","),
      " registrations from ",
      format.default(
        nrow(proofed_data |> 
               dplyr::filter(registration_yr == params$year) |> 
               dplyr::distinct(dl_state)), 
             big.mark = ","),
      " states are sample eligible for the current season."
    ),
    # Captured message outputs from other functions
    purrr::map(
      seq_len(nrow(important_statistics)),
      ~dplyr::slice(important_statistics, .x) |> dplyr::pull())
    ,
    "* We dropped:",
    # Dropped record summaries
    purrr::map(
      seq_len(nrow(n_dropped)),
      ~dplyr::slice(n_dropped, .x) |> dplyr::pull())
    ,
    # Canada summary
    if (canada_records > 0) {
      paste0(
...
if (nrow(pii2 |> dplyr::filter(prop >= 0.1 | n >= 100)) > 0) {
  if (nrow(pii2) == 1) {
    ...
    cat(
      paste0(
      "❌",
      " ", nrow(pii2), " files are missing PII in > 10% or 100+ records."))
  }
  cat("\n\n")
  pii2 |>
    ...
    kableExtra::row_spec(
      seq_len(nrow(pii2)), extra_css = "padding: 3px;")
} else {
...
if (nrow(emails_missing_flagged) > 0) {
  ...
  emails_missing_flagged |>
    ...
    kableExtra::kable_styling(full_width = F) |>
    kableExtra::row_spec(
      seq_len(nrow(emails_missing_flagged)), extra_css = "padding: 3px;")
} else {
...
if (nrow(huntcheck) > 0) {
  ...
  huntcheck |>
    ...
    kableExtra::kable_styling(full_width = F) |>
    kableExtra::row_spec(
      seq_len(nrow(huntcheck)), extra_css = "padding: 3px;")
} else {
...
if (is.null(bagcheck_tbl_non0)) {
  ...
  kableExtra::kbl(
    bagcheck_tbl_non0 |>
      dplyr::mutate(
        nprop = as.numeric(stringr::str_extract(proportion, "[0-9]{1,3}"))) |>
    ...
    kableExtra::row_spec(
      seq_len(nrow(bagcheck_tbl_non0)), extra_css = "padding: 3px;")
  ...
  if (nrow(bagcheck_tbl_non0) == 1) {
    cat(
      paste0(
        ...
        bagcheck_tbl_non0$proportion, " of ",
          ...
          dplyr::mutate(
            value =
              ifelse(
                proportion == "100%",
                proportion,
                paste0(proportion, " (n = ", n, ")"))),
        bagcheck_tbl_non0$dl_state)

    writeLines(
      unlist(
        purrr::map(
          seq_along(bagcheck_split),
          function(x) {
            list(
              paste0("* ", names(bagcheck_split[x])),
              purrr::map(
                seq_len(nrow(bagcheck_split[[x]])),
                function(y) {
                  paste0(
                    "    + ",
                    ...
                    " (expected ",
                    bagcheck_split[[x]]$expected_bag_value[y], ")")
                }
              )
            )
          }
    ...
    purrr::map(
      ...
      function(x) {
        list(
          ...
          purrr::map(
            seq_len(nrow(split_bit[[x]])),
            function(y) {
              paste0(
                "    + ", split_bit[[x]]$n[y], " from ",
...
if (nrow(duplicates) > 0) {
  ...
  cat(
    paste0(
      "❌",
      " Out of ", format.default(nrow(cleaned_data), big.mark = ","),
      " records, ",
...
if (!is.null(rfstate)) {
  cat(
    paste(
      "Values in the table below are reported at the field level (13 fields",
      "per record).")
  )
  kableExtra::kbl(
    rfstate |>
    dplyr::mutate(
      proportion = paste0(round(proportion, 2) * 100, "%"),
    ...
    kableExtra::kable_styling(full_width = F) |>
    kableExtra::row_spec(
      seq_len(nrow(rfstate)), extra_css = "padding: 3px;")
} else {
...
if (!is.null(rffield)) {
  ...
  kableExtra::kbl(
    ...
    kableExtra::kable_styling(full_width = F) |>
    kableExtra::row_spec(
      seq_len(nrow(rffield)), extra_css = "padding: 3px;")
} else {
...
files_received <-
    ...
    dplyr::filter(dl >= 800 & state %in% migbirdHIP:::REF_ABBR_49_STATES) |>
...
if (params$dl == "0800") {
  ...
  migbirdHIP:::SF_HEXMAP |>
    ...
    ggplot2::theme(
      ...
      legend.text = ggplot2::element_text(vjust = -8.5), 
      legend.title = ggplot2::element_text(vjust = 0.2)) +
    ggplot2::guides(
      fill = ggplot2::guide_legend(
        theme = ggplot2::theme(legend.text.position = "top")))
}
```
...
if (length(excluded) > 0) {
  cat(
    paste0(
      "❌",
      " These states submitted files for this download, but no records were",
      " used: ",
    ...
    cat(
      paste(
        "✔️ All download states that submitted files are included in the final",
        "data!")
    )
}
...
if (nrow(record_check) > 0) {
  cat(
    paste0(
      "The following records are missing first name, last name, date of birth,",
      " address AND email, OR city AND zip AND email. They are not included in",
      " the final data."
    )
  )
...
if (nrow(pii_summary) > 0) {
  cat(
    paste0(
      "❌", " There are ", nrow(record_check),
      " total records with missing PII. Below is a summary of the number of ",
      "records missing from each HIP file."
      )
  )
  DT::datatable(
    pii_summary |>
...
## All issue dates received

The figure below shows all issue dates received per state. Issue dates may contain future records. The red lines delineate the download period that spans the last 2 weeks from today.

```{r received_issue_dates_plot}
#| echo: false
#| fig.align: center
...
dates <-
  ...
  dplyr::mutate(issue_date = lubridate::mdy(issue_date)) 

dates |> 
  ggplot2::ggplot() +
  ggplot2::geom_vline(
    ggplot2::aes(
      xintercept = lubridate::today() - 14), 
    color = "red", 
    linetype = "dashed") +
  ggplot2::geom_vline(
    ggplot2::aes(
...
if (nrow(dplyr::filter(issue_assignments, decision != "current")) > 0) {
...
bad_issue_nrow <-
  issue_assignments |>
  dplyr::filter(decision != "current") |>
  dplyr::filter(
    !(dl_state %in% migbirdHIP:::REF_STATES_2SEASON)) |>
  nrow()
  ...
  DT::datatable(
    suppressMessages(
      issue_assignments |>
        ...
        dplyr::left_join(
          migbirdHIP:::REF_DATES |>
            dplyr::select(dl_state = state, issue_end),
          by = "dl_state"
        ) |>
        dplyr::mutate(n = format.default(n, big.mark = ",")) |>
        dplyr::select(
          State = dl_state,
          `Last day of issuance` = issue_end,
          `Number of records` = n
        )
    ),
    options =
...
x_order |>
  ...
  dplyr::mutate(
    ...
    count_errors_label =
      ifelse(
        count_errors > 0,
        format.default(count_errors, big.mark = ",", trim = T),
        0)) |>
  dplyr::ungroup() |>
  ggplot2::ggplot() +
  ggplot2::geom_bar(
    ...
    dplyr::mutate(
      value =
        stringi::stri_enc_toutf8(
          value, is_unknown_8bit = T, validate = T)) |>
    dplyr::group_by(value) |>
    dplyr::summarize(count = dplyr::n()) |>
    dplyr::ungroup() |>
    dplyr::arrange(dplyr::desc(count)) |>
    dplyr::mutate(count = format.default(count, big.mark = ","))
  } else {
  err1 <-
    migbirdHIP::pullErrors(
      proofed_data, field = et$error[1], unique = FALSE) |>
...
if (nrow(zipcheck) > 0) {
  cat(
    paste(
      "The following table shows records in the final data with zip codes that",
      "do not match the address state.")
  )
...
migbirdHIP::errorPlotStates(proofed_data, threshold = 0)
```
...
migbirdHIP::errorPlotStates(final_data, threshold = 0)
...
DT::datatable(
  migbirdHIP::errorTable(proofed_data, loc = "all") |>
    dplyr::arrange(dplyr::desc(error_count))  |>
    dplyr::mutate(error_count = format.default(
      error_count, big.mark = ",", justify = "right"
    )) |>
    dplyr::rename(
      State = dl_state,
      Error = error,
...
DT::datatable(
  migbirdHIP::errorTable(final_data, loc = "all") |>
    dplyr::arrange(dplyr::desc(error_count)) |>
    dplyr::mutate(error_count = format.default(error_count, big.mark = ",")) |>
    dplyr::rename(
      State = dl_state,
      Error = error,
      Count = error_count
