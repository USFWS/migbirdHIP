...
dl_data <- 
  catch_messages(migbirdHIP::read_hip(params$temp_path)) |> 
...
current_data <- catch_messages(migbirdHIP::issueCheck(cleaned_data, params$year, write = F))
...
blanks <-
  tibble::tibble(
    list.files(
      params$temp_path, recursive = F, pattern = "*\\.txt$", 
...
bullets <- 
  c(
    ...
    paste0(
      "* This cycle, ", 
      suppressWarnings(migbirdHIP::sumLines(params$temp_path)), 
...
files_received <- 
  ...
  list.files(
    stringr::str_remove(params$temp_path, "\\/D.+"), recursive = T) |> 
  tibble::as_tibble() |> 
...
Records below had their registration year edited to be compatible for this HIP season. They were also copied and exported for next HIP season.
...
if(nrow(issue_assignments |> dplyr::filter(decision == "copy")) > 0){
  ...
  cat("✔️ No records to change/copy.")
}
...
Records below have an issue date and registration year indicating they should be set aside for next year.
...
Records below have an issue date and registration year indicating they qualify for a previous HIP season; these records were set aside and are not be eligible to be sampled.
