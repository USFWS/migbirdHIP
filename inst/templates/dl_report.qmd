...
dl_data <- 
  migbirdHIP::read_hip(params$comp_path) |> 
...
cleaned_data <- migbirdHIP::clean(unshifted_data)

rm(unshifted_data)

current_data <- migbirdHIP::issueCheck(cleaned_data, params$year, write = F)

fixed_data <- migbirdHIP::fixDuplicates(current_data)
...
DT::datatable(migbirdHIP::findDuplicates(fixed_data, return = "table"))
...
DT::datatable(
  migbirdHIP::strataCheck(final_data),
  extensions = "Buttons",
  options = 
    list(
      dom = "Bfrtip",
...
et <- migbirdHIP::errorTable(proofed_data, loc = "none") |> dplyr::arrange(dplyr::desc(error_count))
...
if(et$error[1] != "birth_date"){
  err1 <- 
    migbirdHIP::pullErrors(
      proofed_data, error = et$error[1], distinct = FALSE) |> 
    tibble::as_tibble() |> 
    dplyr::mutate(
...
if(et$error[2] != "birth_date"){
  err2 <- 
    migbirdHIP::pullErrors(
      proofed_data, error = et$error[2], distinct = FALSE) |> 
    tibble::as_tibble() |> 
    dplyr::mutate(
      value = 
...
if(et$error[3] != "birth_date"){
  err3 <- 
    migbirdHIP::pullErrors(
      proofed_data, error = et$error[3], distinct = FALSE) |> 
    tibble::as_tibble() |> 
    dplyr::mutate(
      value = 
        stringi::stri_enc_toutf8(value, is_unknown_8bit = T, validate = T)) |>
...
migbirdHIP::redFlags(proofed_data, type = "state", threshold = 0.01)
```
...
migbirdHIP::redFlags(proofed_data, type = "field", threshold = 0.01)
```
