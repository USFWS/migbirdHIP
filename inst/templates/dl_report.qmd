  ...
  dplyr::mutate(
    ...
    type = 
      dplyr::case_when(
        ...
        # params$year
        issue_date %within% 
          lubridate::interval(issue_start, issue_end) & 
        ...
        issue_date < issue_start & 
          registration_yr == as.character(params$year) ~ 
          "issue date before the first day of issuance",
        # Issue date is before the first day of the hunting season &
        # registration year is != params$year
        issue_date < issue_start & 
          registration_yr != as.character(params$year) ~ 
          paste0(
            "issue date before the first day of issuance, and",
            " registration year is"),
        # Issue date is after issue_end
        issue_date > issue_end & 
          registration_yr == as.character(params$year) ~ 
          "issue date after end of issue window",
        # Repeat above but with different registration yr
        # Issue date is after issue_end
        issue_date > issue_end & 
          registration_yr != as.character(params$year) ~ 
          paste0(
...
important_statistics <- 
  dplyr::bind_rows(
    ...
    dplyr::filter(
      !stringr::str_detect(
        value, 
        "are blank|ID fields|provided state|write-out|past records|0 future|issue_date in the|Mississippi|100% of emails|00/00/0000")
    ) |> 
    dplyr::mutate(
      value = 
...
idb2 <-
  ...
  dplyr::left_join(
    migbirdHIP:::hip_bags_ref |>
      ...
      dplyr::bind_rows(
        migbirdHIP:::pmt_files |> 
          dplyr::rename(repeated_value = value, spp1 = spp)) |>
      dplyr::distinct() |>
      dplyr::select(-repeated_value) |> 
      dplyr::mutate(flag1 = "no season"),
    by = c("dl_state", "spp1")
  ) |> 
  dplyr::left_join(
    migbirdHIP:::hip_bags_ref |>
      ...
      dplyr::bind_rows(
        migbirdHIP:::pmt_files |> 
          dplyr::rename(repeated_value = value, spp2 = spp)) |>
      dplyr::distinct() |>
      dplyr::select(-repeated_value) |> 
      dplyr::mutate(flag2 = "no season"),
    by = c("dl_state", "spp2")
  ) |> 
  dplyr::filter(is.na(flag1) & is.na(flag2)) |> 
  dplyr::select(-c("flag1", "flag2"))
...
if(nrow(idb2) > 0) {
  ...
  cat(
    paste0(
      "💡", " Likely species combinations not included (coots_snipe and rails_gallinules, brant and seaducks)."))
  cat("\n\n")
  if(nrow(idb2) == 1) { 
    cat(
      paste0(
...
Records below did not have an issue date and/or registration year that were current.
...
if(nrow(dplyr::filter(issue_assignments, decision != "nochange") |> dplyr::filter(!(dl_state %in% states_twoseason & decision == "copy"))) > 0){
  stamp_format <- lubridate::stamp("Apr 15, 1993")
  
  bad_issue_table <-
    issue_assignments |>
    dplyr::filter(decision != "nochange") |>
    dplyr::filter(!(dl_state %in% states_twoseason & decision == "copy")) |> 
    dplyr::select(dl_state, source_file, issue_date, registration_yr) |>
    dplyr::left_join(
      migbirdHIP:::licenses_ref |>
        dplyr::select(
...
Records below have an issue date and registration year indicating they qualify for a previous HIP season; these records are not eligible to be sampled.
