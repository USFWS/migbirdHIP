...
DT::datatable(findDuplicates(proofed_data, return = "table"))
```

## Strata

```{r stratacheck, echo = FALSE}
DT::datatable(strataCheck(proofed_data))
```
...
DT::datatable(
  migbirdHIP::validate(proofed_data, type = "horizontal") %>% 
...
DT::datatable(migbirdHIP::validate(proofed_data, type = "vertical"))
```
...
DT::datatable(
  ...
  select(-ducks_bag) %>% 
  distinct() %>% 
  mutate(zero_ducks_prop = round(zero_rows / dl_rows, 2)) %>% 
  arrange(desc(zero_ducks_prop)) %>% 
  relocate(zero_rows, .before = "dl_rows")
)
```
...
DT::datatable(outOfStateHunters(proofed_data, output = "table"))
```
