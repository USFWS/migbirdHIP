...
cleaned_data <- 
  clean(unshifted_data) %>% 
  # Edit the persistent issue of one-season states sending yr+1 when it should just be yr
  mutate(
    registration_yr =
      ifelse(
...
if(future_file %in% list.files(future_path)){
  future_data <-
    read_csv(paste0(future_path, future_file), show_col_types = F)
...
if(past_file %in% list.files(past_path)){
  past_data <-
    read_csv(paste0(past_path, past_file), show_col_types = F)
...
record_check <- 
  ...
  # email, OR city, zip AND email are missing -- records discarded because
  # these are needed to identify individuals
  # Discard additional records if they are missing elements of an address AND email
  filter(
    if_any(c("firstname", "lastname", "state", "birth_date"), ~is.na(.x))|
      if_all(c("address", "email"), ~is.na(.x))|
      if_all(c("city", "zip", "email"), ~is.na(.x)))
...
```{r missingemail, echo = FALSE, fig.align = "center"}
...
final_data %>% 
  ...
  mutate(
    ...
    Category = 
      case_when(
        !is.na(email) & !str_detect(errors, "email") ~ "Good",
        !is.na(email) & str_detect(errors, "email") ~ "Incorrect",
        is.na(email) ~ "Missing",
        TRUE ~ NA_character_)) %>% 
  select(-c("email", "errors")) %>% 
  filter(Category != "Good") %>% 
  group_by(dl_state) %>% 
  mutate(total_rows = n()) %>% 
  ungroup() %>% 
  group_by(dl_state, Category) %>% 
  mutate(proportion = n()/nrow(final_data)) %>% 
  ungroup() %>% 
  distinct() %>% 
  group_by(dl_state) %>% 
  mutate(label = ifelse(row_number() == 1, total_rows, NA)) %>% 
  ungroup() %>% 
  # Plot
  ggplot() +
  geom_bar(
...
if(nrow(h_valid) == 0){
  ...
  kableExtra::kbl(
    h_valid %>% 
      ...
      arrange(desc(binned_proportion)) %>% 
