...
Below is a summary of all values in "hunt_mig_birds" that are not equal to 2.
...
The following records are missing first name, last name, date of birth, address, city, and/or state of residence.
...
Abbreviated states, districts, territories, etc that are considered valid include: all 50 states, DC, HI, AS (American Samoa), GU (Guam), MP (Northern Mariana Islands), PR (Puerto Rico), VI (Virgin Islands), UM (US Minor Outlying Islands), AA (US Armed Forces - America), AE (US Armed Forces - Europe), and AP (US Armed Forces - Pacific).
  ...
  mutate(
    valid = 
      case_when(
        ...
        !(state %in% acceptable_residences) ~ "invalid", 
        TRUE ~ NA_character_)) %>%
  filter(!is.na(valid)) %>%
  arrange(desc(valid))
...
all0s <- 
  ...
  filter(
    ducks_bag == "0" &
      ...
      brant == "0" &
      seaducks == "0") %>%
  select(record_key, hunt_mig_birds:seaducks)
