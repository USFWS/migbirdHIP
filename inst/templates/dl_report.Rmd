...
There are `r canada_records` records in this download from Canadian hunters. A total of `r zip_check` records contain zip codes that are not correct based on the provided state of residence.

Below is a table summarizing the total of number of records per download state.

```{r sum_per_state, echo = FALSE, message = FALSE}

DT::datatable(fixed_data %>% 
                select(dl_state) %>% 
                group_by(dl_state) %>% 
                summarize(sum = n()) %>% 
                ungroup())
...
DT::datatable(fixed_data %>% 
                select(registration_yr) %>% 
                group_by(registration_yr) %>% 
                summarize(n = n()) %>% 
                ungroup())
...
Below is a summary of all values in the hunt_mig_birds (aka "HuntY") field.
...
```{r erpf_c, echo = FALSE, fig.align = "center"}
